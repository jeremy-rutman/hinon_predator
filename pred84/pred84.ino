...
uint8_t CURRENT_ARM_DIRECTION = 0;
...
// arm(CLOSED);
  ...
  Serial.println("zeroing all");
    Serial.println("quit movie quit");
...
int domovie()
{
  ...
  Serial.println("beginning the mov");
  Serial.flush();
  PAUSE_TIME=0;
  STATUS = 0;
...
//  if (mydelay(INIT_DELAY*1000)) return;
  const int8_t EXTRA_TIME = -10;  //delay time for syncing movie to command from control computer (seconds)
  i=0;

//the timeline !

  //zero stuff out and wait 
//lower arm (its its not alrSECONDSSECONDSeady down)
...
//delay(100);
 ...
 index = 0 ;
  ...
  int max_index = sizeof(timeline)/sizeof(timeline[0]);
    Serial.print("movi time, index:max index=");Serial.print(index);Serial.print(":");Serial.println(max_index);
    Serial.flush();
  while(index<max_index)
  {
    ...
    t_seconds =(unsigned long)( t_millis/1000);
      Serial.println("ok3:");
    Serial.flush();
    int dt = (int)t_seconds-(int)start_seconds - (int)PAUSE_TIME;
     Serial.println("ok4:");
    Serial.flush();
    Serial.print("curtime:"); Serial.print(dt);
    Serial.print(" nexttime:"); Serial.print((int)timeline[index]-(int)PREDELAY);
    Serial.print(" pausetime:");Serial.print((int)PAUSE_TIME);
...
int buttoncheck()
{
  ...
  {
    STATUS = 0;
    if (PAUSE_STATE == HIGH) PAUSE_STATE = LOW;
     else PAUSE_STATE = HIGH;
     
     Serial.print("ps button on, ps status ");Serial.println(PAUSE_STATE);
     Serial.flush();
...
temp =0;
 ...
 if (CURRENT_ARM_DIRECTION)
{
  temp = CURRENT_ARM_DIRECTION;
  arm(CLOSED);
}  
  while (PAUSE_STATE == HIGH)
 {
   delay(200);
    Serial.println("doing ps");
    Serial.flush();
    buttoncheck();
   //do nothing
 }
 ...
 int dt = end_time-curtime;
 PAUSE_TIME=PAUSE_TIME+(int)(dt/1000);
 Serial.print("tot pause time:");Serial.println(PAUSE_TIME); 
 Serial.flush();
 if(temp)  arm(temp);
...
mydelay(150); 
  ...
  switch (STATUS) 
  {
    ...
    case MOVIE:
      STATUS = 0;
      Serial.println("gggot MOVi command");
      Serial.flush();
