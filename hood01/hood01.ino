...
int servoMins[] =    {4600,5800,5700,5200,6000,5700,5900,4000,4200,4300,3700,3900};
int servoCenters[] = {5900,6100,6100,5700,7311,6277,6350,6300,7200,5900,5822,6000};
int servoMaxes[] =   {6600,6400,6700,6200,7312,7000,6800,7800,7200,7800,6500,8100};
...
uint8_t MOTOR = 12;
uint8_t LED = 13;
uint8_t ARM = 0;  //this is the servo for hinon's "servo H-bridge"
uint8_t ROLLYAW = 15;  //not being used - for simultaneous roll/yaw

//arduino pins for non-servo parts of system - running low-side nmos switches on arduino shield
uint8_t MOTOR_PIN = 3;      
uint8_t LED_PIN = 6;
...
//  pinMode(ARM_FWD_PIN, OUTPUT);
 // pinMode(ARM_REV_PIN, OUTPUT);
...
int  hood(int command)
{
  ...
  if (command == OPEN)
  {
    //yinnon   change here - move_rel(servonum,start,finish,delay(ms))
      if(move_rel(servonum,0,100,25)) return 1 ;
  }
  else   //close
  {
    //and here
      if(move_rel(servonum,100,0,25)) return 1;
  }
}
...
//     digitalWrite(ARM_REV_PIN,LOW);
//     digitalWrite(ARM_FWD_PIN,HIGH);
...
//     digitalWrite(ARM_FWD_PIN,LOW);
 //    digitalWrite(ARM_REV_PIN,HIGH);
//      digitalWrite(A0,LOW);
...
int showoff()
{
  Serial.println("starting showoff");
 delay(100);
...
//initialize movie timeline with all zeros. if this length is too short (less than numbner of movie commands) you are writing into unprotected memory ...
  ...
  const int8_t EXTRA_TIME = -8;  //delay time for syncing movie to command from control computer (seconds)
  i=0;
...
//takeoff
  ...
  tl_args3[i]=20;  //'slowness' - # of mSec pause between each servo step  (integer steps bet. start and end, so tot time taken (in mSec) is (start-end)*slowness
  i++;   
  timeline[i]=20+EXTRA_TIME;
...
//delay(100);
 ...
 index = 0 ;
    t_millis=millis();
    start_seconds =(unsigned long)( t_millis/1000);

  int max_index = sizeof(timeline)/sizeof(timeline[0])-1;
  ...
  //    if(docheck())return(1);
    ...
    t_millis=millis();
    t_seconds =(unsigned long)( t_millis/1000);
...
{
//  movie1();
//position_rel(HOOD,50);
//  delay(3000);
//position_rel(HOOD,30);
//delay(3000);
    hood(OPEN);
  delay(1000);
  hood(CLOSED);
  delay(1000);
 // movie1();
//move_rel(MTD_PI,50,100,100);
//delay(1000);
//move_rel(MTD_PI,100,0,100);
//delay(1000);
//move_rel(MTD_PI,0,50,100);
//delay(1000);
// showoff();
...
//movie1();
//Serial.println("mtdpi 20000 ");
// maestro.setSpeed(MTD_PI,100);
// position_rel(MTD_PI,0) ;
//  position_rel(MTD_PI,0) ;

 // delay(2000);
//Serial.println("mtdpi 0 ");
//  maestro.setSpeed(MTD_PI,150);
 // position_rel(MTD_PI,100) ;
//delay(2000);
  ...
  switch (STATUS) 
  {
    ...
    case MOVIE:
      STATUS = 0;
      Serial.println("got MOVIE command");
      movie1();
   //   zero_all();
      break;
    case SCREENSAVER:
