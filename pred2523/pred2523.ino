...
int servoMins[] =    {4600,5650,5700,4000,6000,5000,5900,4000,4200,4300,3700,3900};
int servoCenters[] = {5900,5925,6100,4500,7000,6500,6350,6300,7200,5900,5822,6000};
int servoMaxes[] =   {6600,6250,6700,8000,7312,7100,6800,7800,7200,7800,6500,8100};
//SERVO NUMBERS - channel numbers out of the maestro servo controller and into hinon's system
const uint8_t ROLL = 1;
const uint8_t PIT = 2;  //pitch
const uint8_t YAW = 3;
const uint8_t FLP = 4; //was 4,5
const uint8_t AIL = 5; //was 6,7 
const uint8_t RUD = 6; //was 8    
const uint8_t ELE = 7; // reversed
const uint8_t HOOD = 8; //was 11   
const uint8_t MTD_YA = 9;     //12 
const uint8_t MTD_PI = 10; //13     
const uint8_t WHL = 11; //on off

//numbers to refer to other parts of system 
const uint8_t MOTOR = 12;
const uint8_t LED = 13;
const uint8_t ARM = 0;  //this is the servo for hinon's "servo Hbridge"
const uint8_t ROLLYAW = 15;  //not being used  for simultaneous roll/yaw

//arduino pins for nonservo parts of system  running lowside nmos switches on arduino shield
const uint8_t MOTOR_PIN = 3;      
const uint8_t LED_PIN = 6;
const uint8_t ARM_FWD_1 = 4;  //check these
const uint8_t ARM_FWD_2 = 7;
const uint8_t ARM_REV_1 = 5;
const uint8_t ARM_REV_2 = 2;
...
int movie1()
{
  ...
  STATUS = 0;
  int8_t EXTRA_TIME = 1;  //delay time for syncing movie to command from control computer (seconds)
  ...
  RETURN_TO = WAITING;
  uint8_t movielength = 85;  //number of actiofns in movie  warning if this isn't high enough, you are writing into random memory 
...
// slight descent over clouds- SAR
...
//camera on target 15
  timeline[i]=180+57+EXTRA_TIME;
  tl_servos[i]=MTD_YA;
  tl_args1[i]=50;
  tl_args2[i]=40;
  tl_args3[i]=30;
  i++;     
  timeline[i]=180+58+EXTRA_TIME;
  tl_servos[i]=MTD_PI;
   tl_args1[i]=70;
  tl_args2[i]=0;
  tl_args3[i]=20;
...
//delay(100);
 ...
 index = 0 ;
  ...
  while(index<i)
  {
    ...
    Serial.print("c:"); Serial.print(dt);
 //   Serial.print(" nexttime:"); Serial.print((int)timeline[index]-(int)PREDELAY);
    Serial.print(" p :"); Serial.print((int)timeline[index]);
    Serial.print(" i :"); Serial.println((int)index);
 
    if ((int)dt>=(int)((int)timeline[index]-(int)PREDELAY))
    {
        if(docheck())return(1);

      Serial.print("servo "); Serial.println(tl_servos[index]);
      Serial.flush();
